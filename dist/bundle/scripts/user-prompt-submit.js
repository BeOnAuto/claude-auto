...
`);
  ...
  function exists() {
    return fs4.existsSync(stateFile);
  }
  return {
    exists,
    read,
...
function scanReminders(remindersDir) {
  if (!fs5.existsSync(remindersDir)) {
    return [];
  }
  return fs5.readdirSync(remindersDir).filter((f) => f.endsWith(".md"));
}
...
function loadReminders(dirs, context, overrides) {
  ...
  for (const dir of dirs) {
    ...
    for (const filename of filenames) {
      ...
      seen.add(filename);
      const content = fs5.readFileSync(path5.join(dir, filename), "utf8");
...
var FIRST_SETUP_MESSAGE = `CRITICAL INSTRUCTION \u2014 DO THIS BEFORE ANYTHING ELSE:
