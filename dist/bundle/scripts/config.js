...
function scanReminders(remindersDir) {
  if (!fs2.existsSync(remindersDir)) {
    return [];
  }
  return fs2.readdirSync(remindersDir).filter((f) => f.endsWith(".md"));
}
...
function loadAllValidatorMeta(filePath) {
  const fileContent = fs3.readFileSync(filePath, "utf8");
  const { data } = (0, import_gray_matter2.default)(fileContent);
  return {
    name: data.name,
...
function setConfigValue(paths, keyPath, value) {
  const hookState = createHookState(paths.autoDir);
  const state = hookState.read();
  const parsed = parseValue(value);
  setNestedValue(state, keyPath, parsed);
  hookState.write(state);
  return state;
}
function listValidators(paths, overrides) {
  ...
  for (const dir of paths.validatorsDirs) {
    ...
    for (const file of fs4.readdirSync(dir)) {
      ...
      const meta = loadAllValidatorMeta(path2.join(dir, file));
      if (seen.has(meta.name)) continue;
      seen.add(meta.name);
      const override = state.validators[meta.name];
      const enabled = override !== void 0 ? override.enabled : meta.enabled !== false;
      statuses.push({
        name: meta.name,
...
function listReminders(paths, overrides) {
  const state = overrides ?? createHookState(paths.autoDir).read().overrides;
  const statuses = [];
  const seen = /* @__PURE__ */ new Set();
  for (const dir of paths.remindersDirs) {
    ...
    for (const filename of filenames) {
      ...
      seen.add(filename);
      const content = fs4.readFileSync(path2.join(dir, filename), "utf8");
...
function toggleValidator(paths, name, enabled) {
  const hookState = createHookState(paths.autoDir);
  const state = hookState.read();
...
function toggleReminder(paths, name, enabled) {
  const hookState = createHookState(paths.autoDir);
  const state = hookState.read();
...
function setReminderPriority(paths, name, priority) {
  const hookState = createHookState(paths.autoDir);
  const state = hookState.read();
  if (!state.overrides.reminders[name]) {
    state.overrides.reminders[name] = {};
...
function addReminder(paths, name, options2) {
  const projectRemindersDir = paths.remindersDirs[paths.remindersDirs.length - 1];
  if (!fs4.existsSync(projectRemindersDir)) {
    fs4.mkdirSync(projectRemindersDir, { recursive: true });
  }
  const frontmatter = {};
  if (options2.hook) {
    frontmatter.when = { hook: options2.hook };
  }
  if (options2.priority !== void 0) {
    frontmatter.priority = options2.priority;
  }
  const yaml2 = Object.keys(frontmatter).length > 0 ? `---
${formatYaml(frontmatter)}---

` : "";
  const filePath = path2.join(projectRemindersDir, `${name}.md`);
  fs4.writeFileSync(filePath, `${yaml2}${options2.content}
`);
  return filePath;
}
function removeOverride(paths, type, name) {
  const hookState = createHookState(paths.autoDir);
  const state = hookState.read();
...
function parseReminderAddArgs(remaining) {
  let hook;
  let priority;
  let content = "";
...
function handleValidators(paths) {
  const action = args[1];
  if (!action) {
...
function handleReminders(paths) {
  const action = args[1];
  if (!action) {
  ...
  switch (action) {
    ...
    case "reset":
      ...
      break;
    case "add": {
      const options2 = parseReminderAddArgs(args.slice(3));
      const filePath = addReminder(paths, name, options2);
