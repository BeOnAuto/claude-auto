...
describe('activity-logger', () => {
  ...
  it('writes to .ketchup/logs/activity.log', () => {
    ...
    activityLog(ketchupDir, 'session-123', 'test-hook', 'test message');
  ...
  it('formats log with short date+time and last 8 chars of session ID', () => {
    ...
    activityLog(ketchupDir, 'abc12345-6789-defg', 'session-start', 'loaded reminders');
  ...
  it('appends multiple log entries', () => {
    ...
    activityLog(ketchupDir, 'session-1', 'hook-a', 'message 1');
    activityLog(ketchupDir, 'session-2', 'hook-b', 'message 2');
  ...
  it('filters by KETCHUP_LOG env when set to specific hook', () => {
    ...
    activityLog(ketchupDir, 'session-1', 'session-start', 'started');
    activityLog(ketchupDir, 'session-1', 'pre-tool-use', 'allowed: file.ts');
  ...
  it('allows multiple comma-separated patterns in KETCHUP_LOG', () => {
    ...
    activityLog(ketchupDir, 'session-1', 'session-start', 'started');
    activityLog(ketchupDir, 'session-1', 'pre-tool-use', 'block: denied');
    activityLog(ketchupDir, 'session-1', 'pre-tool-use', 'allowed: file.ts');
  ...
  it('excludes patterns prefixed with -', () => {
    ...
    activityLog(ketchupDir, 'session-1', 'session-start', 'started');
    activityLog(ketchupDir, 'session-1', 'pre-tool-use', 'blocked: secret.ts');
    activityLog(ketchupDir, 'session-1', 'pre-tool-use', 'allowed: file.ts');
